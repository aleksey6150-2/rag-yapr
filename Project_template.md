...
Подробный отчёт: [Аналитика качества](task7/task7_README.md)

Из базы удалены 3 ключевых документа (Void Core, Xarn Velgor, Synth Flux) → gapped-индекс (33 документа, 66 чанков). Golden set — 13 вопросов (8 known + 5 gap). Результат: **known 8/8 (100%)**, **gap 1/5 (20%)**. Ключевое наблюдение: информация «размазана» по смежным документам — бот собирает ответы даже при удалении основного файла. Логирование в JSONL с полями: запрос, timestamp, sources, terms_found, success, latency. Sequence diagram в PlantUML.
